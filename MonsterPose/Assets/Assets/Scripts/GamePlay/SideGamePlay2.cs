...
public class SideGamePlay2 : MonoBehaviour
{
        ...
        if (!locked && IsWithinBoxCollider() && !losePanel.activeInHierarchy)
        {
            if (Input.touchCount > 0)
            {
                ...
                if (touch.phase == TouchPhase.Ended && isTouch == 1)
                {
                    ...
                    {
                        if (Mathf.Abs(transform.position.x - rightPos.position.x) <= 0.8f && Mathf.Abs(transform.position.y - rightPos.position.y) <= 0.8f)
                        {
